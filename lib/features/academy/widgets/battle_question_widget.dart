  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Column(
          children: [
            const Text('AI (상대방)', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: widget.question.aiHoleCards.map((c) => Padding(
                padding: const EdgeInsets.symmetric(horizontal: 4.0),
                child: SizedBox(
                  width: 60,
                  height: 84,
                  child: PlayingCardView(card: c, showBack: !_showAiCards),
                ).animate(target: _showAiCards ? 1 : 0).flipH(duration: 400.ms),
              )).toList(),
            ),
          ],
        ...
        if (widget.question.communityCards.isNotEmpty)
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: widget.question.communityCards.map((c) => Padding(
          ...
          const Text('VS', style: TextStyle(color: Colors.amber, fontSize: 36, fontWeight: FontWeight.w900, fontStyle: FontStyle.italic))
        ...
        Column(
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: widget.question.userHoleCards.map((c) => Padding(
                padding: const EdgeInsets.symmetric(horizontal: 4.0),
                child: SizedBox(
                  width: 70,
                  height: 98,
            ...
            const SizedBox(height: 8),
            const Text('내 카드', style: TextStyle(color: Colors.greenAccent, fontWeight: FontWeight.w900, fontSize: 18)),
          ],
        ),
            ...
            child: ElevatedButton(
              ...
              child: const Row(
                ...
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('승부하기!', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white)),
                ],
              ),
            ),
          ).animate(onPlay: (c) => c.repeat(reverse: true))
