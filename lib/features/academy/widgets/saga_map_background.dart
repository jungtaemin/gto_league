...
class _CasualCloudPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawShape(canvas, size, paint,
        xOffset: 0.2, startYOffset: 0.1, radius: 40, speed: 1.0);
    _drawShape(canvas, size, paint,
        xOffset: 0.8, startYOffset: 0.4, radius: 60, speed: 0.6);
    _drawShape(canvas, size, paint,
        xOffset: 0.5, startYOffset: 0.8, radius: 30, speed: 1.2);
    _drawShape(canvas, size, paint,
        xOffset: 0.1, startYOffset: 0.7, radius: 80, speed: 0.4);
    _drawShape(canvas, size, paint,
        xOffset: 0.9, startYOffset: 0.0, radius: 50, speed: 0.8);
  }
  ...
  void _drawShape(
    ...
    final y =
        movingY < 0 ? size.height * (1.0 + movingY) : size.height * movingY;
    final x = size.width * xOffset;

    // 약간 둥글둥글 겹친 구름 형태
    canvas.drawCircle(Offset(x, y), radius, paint);
    canvas.drawCircle(
        Offset(x + radius * 0.8, y + radius * 0.2), radius * 0.7, paint);
    canvas.drawCircle(
        Offset(x - radius * 0.6, y + radius * 0.3), radius * 0.9, paint);
  }
